...
int main(int argc, char const *argv[]) {
  ...
  gray_image = (unsigned char **)malloc(height * sizeof(unsigned char*));
  if (gray_image   == NULL)
  {
  ...
  for (int i = 0; i < height; i++)
  {
    gray_image [i] = (unsigned char *)malloc(width * sizeof(unsigned char));
  ...
  filter_image = (unsigned char **)malloc(height * sizeof(unsigned char*));
  ...
  for (int i = 0; i < height; i++)
  {
    filter_image[i] = (unsigned char *)calloc(width, sizeof(unsigned char));
...
void ReadRecord(int fd, unsigned char *buffer)
{
...
void RGB2GrayScale(int width, int height)
{
  for (int i = 0; i < height; i++)
    for (int j = 0; j < width; j++)
      gray_image[i][j] =  (int)((0.3 * image[i][j].R) + (0.59 * image[i][j].G) + (0.11 * image[i][j].B));
